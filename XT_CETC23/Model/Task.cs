...
namespace XT_CETC23.DataCom
{
    class Task 
    {
        ...
        public int CreateTask(string ProductID,int FrameLocation,int SalverLocation,string ProductType,string CurrentStation, bool StationSign,string ProductChectResult,String BatchID, int CabinetID)
        {
            lock (lockObject)
            {
                DataTable dt = DataBase.GetInstanse().DBQuery("select * from dbo.MTR where CabinetID = " + CabinetID);
                ...
                DataTable dt_Mtr = DataBase.GetInstanse().DBQuery("select * from dbo.MTR");
                ...
                string tmpText = "insert into dbo.MTR(ProductID,FrameLocation,SalverLocation,ProductType,CurrentStation,StationSign,ProductCheckResult,BasicID,BeginTime,CabinetID)values('"
                    ...
                    + DateTime.Now + "',"
                    + CabinetID + ")";
                DataBase.GetInstanse().DBInsert("insert into dbo.MTR(ProductID,FrameLocation,SalverLocation,ProductType,CurrentStation,StationSign,ProductCheckResult,BasicID,BeginTime,CabinetID)values('"
                    + ProductID + "',"
                    + FrameLocation + ","
                    + SalverLocation + ",'"
                    + ProductType + "','"
